...
export const getQuestions = async (
  ...
  // FIXME: add recommended filter (we are not using it for now)
  ...
  try {
    ...
    const questions = await Question.find(filterQuery)
      ...
      // lean is used to return the documents as a plain JavaScript object rather than a mongoose document
      .lean()
      .sort(sortCriteria)
      .skip(skip)
      .limit(limit);
    ...
    return {
      success: true,
      // We use JSON.parse(JSON.stringify(...)) to ensure the data is serializable and compatible with Next.js server actions. This helps prevent errors when passing large payloads through server actions.
